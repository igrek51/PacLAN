...
void GameEngine::logic_game(){
        ...
        if(players[i]->can_move==0){
            if(App::network->client){ //predykcja ruchu gracza zanim przyjdzie pakiet dla klienta
                ...
                if(Config::move_while_respawn || players[i]->respawn==0){
                    ...
                    check_next(players[i], players[i]->moving, players[i]->direction, players[i]->next_direction);
                }else{
                    players[i]->moving = 0;
                }
                //wykonanie ruchu
    ...
    if(mode==MODE_ZOMBIE){
        ...
        if(item_count(I_LARGEDOT)==0 && item_count(I_SMALLDOT)<= Config::zombie_dots_threshold ){
    ...
    for(unsigned int i=0; i<players.size(); i++){
        ...
        if(players[i]->can_move==0){
            ...
            if(next_grid(players[i],-1)=='0'){
                ...
                xymap_to_xy(players[i]);
                players[i]->move();
                synchro<<"021 "<<i<<" "<<players[i]->xmap<<" "<<players[i]->ymap<<"\r";
            }else{
                //płynne przesunięcie na ekranie
                xymap_to_xy(players[i]);
                players[i]->move();
                if(players[i]->prev_moving!=players[i]->moving || players[i]->prev_direction!=players[i]->direction){
                    synchro<<"020 "<<i<<" "<<players[i]->xmap<<" "<<players[i]->ymap<<"\r";
                }
            }
            if(players[i]->prev_moving!=players[i]->moving || players[i]->prev_direction!=players[i]->direction){
                synchro<<"022 "<<i<<" "<<players[i]->moving<<" "<<players[i]->direction<<"\r";
            }
        ...
        for(unsigned int j=i+1; j<players.size(); j++){ // i < j
            ...
            if(is_collision(players[i],players[j])){
                ...
                if(players[i]->subclass + players[j]->subclass == 1){ //różne klasy (0 i 1)
                    ...
                    if((players[i]->subclass==P_GHOST&&eating==0)||(players[i]->subclass==P_PACMAN&&eating>0)){
                        killer = i;
                        killed = j;
                    }
        ...
        if(players[i]->subclass==P_PACMAN && players[i]->respawn==0){ //jeśli jest pacmanem
            for(unsigned int j=0; j<items.size(); j++){ //dla obiektu j
                if(is_collision(players[i],items[j])){
                    ...
                    synchro<<"041 "<<items[j]->subclass<<" "<<items[j]->xmap<<" "<<items[j]->ymap<<"\r";
                    delete items[j];
        ...
        if(key==SDLK_ESCAPE){
            ...
            if(menu==MENU_SUBCLASS){
                ...
                if(keystate[SDL_SCANCODE_R]){
                    menu_color.r = (menu_color.r-5)%256;
                }
                if(keystate[SDL_SCANCODE_G]){
                    menu_color.g = (menu_color.g-5)%256;
                }
