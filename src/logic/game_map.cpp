...
void GameEngine::check_next(Player* player1, int& moving, int& direction, int next_direction) {
    if (player1->next_moving == 0) {
        ...
        if (!(next_grid(player1, next_direction) == '-' &&
              (player1->subclass == P_PACMAN || eating >= 1))) {
    ...
    if (moving == 1) {
        if (next_grid(player1, direction) == 'x' || (next_grid(player1, direction) == '-' &&
                                                     player1->subclass ==
                                                     P_PACMAN)) { //pola dostępne tylko dla duszków
            moving = 0;
        }
    }
}
