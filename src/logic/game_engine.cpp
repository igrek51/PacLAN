...
GameEngine::GameEngine(App* app, Graphics* graphics) {
    this->app = app;
...
void GameEngine::logic(volatile int& logic_cycles) {
    ...
    cycles++;
    while (SDL_PollEvent(&event) && !app->exiting()) { //pętla komunikatów SDL
        ...
        if (event.type == SDL_WINDOWEVENT) {
            if (event.window.event == SDL_WINDOWEVENT_MOVED) {
                Log::debug(
                        "Zmiana położenia okna (zatrzymana aplikacja) - reset licznika logicznego");
                logic_cycles = 1;
            }
...
Player* GameEngine::change_subclass(int index, int subclass) {
    ...
    Player* nowy;
    if (subclass == P_GHOST) {
        nowy = new Ghost(players[index]->xmap, players[index]->ymap, players[index]->color,
                         players[index]->name, players[index]->controlby, this, graphics);
    } else if (subclass == P_PACMAN) {
        nowy = new Pacman(players[index]->xmap, players[index]->ymap, players[index]->color,
                          players[index]->name, players[index]->controlby, this, graphics);
    }
...
void GameEngine::synchronize_players(int to) {
    ...
    stringstream synchro2;
    for (unsigned int i = 0; i < players.size(); i++) {
        synchro2 << "021 " << i << " " << players[i]->xmap << " " << players[i]->ymap <<
        "\r"; //przemieszczenie bezwzględne
        synchro2 << "022 " << i << " " << players[i]->moving << " " << players[i]->direction <<
        "\r";
    }
...
Player* GameEngine::add_player(int subclass, string name, SDL_Color color, int controlby,
                               int ai_level) {
    if (controlby == P_AI) {
    ...
    if (network->server) {
        stringstream synchro2;
        synchro2 << "030 " << subclass << " " << (int) color.r << " " << (int) color.g << " " <<
        (int) color.b << " " << name << "\r";
        synchro2 << "021 " << players.size() - 1 << " " << xmap << " " << ymap <<
        "\r";//bezwzględne umieszczenie na mapie
        synchro2 << "022 " << players.size() - 1 << " " << nowy->moving << " " << nowy->direction <<
        "\r";
        network_send_to_clients(synchro2.str());
    }
    return nowy;
...
void GameEngine::kill_player(int index) {
    ...
    graphics->animations.push_back(
            new DeathAnimation(round_to_int(players[index]->a_x), round_to_int(players[index]->a_y),
                               to_copy, players[index]->color, clip, graphics));
    synchro << "050 " << index << " " << round_to_int(players[index]->a_x) << " " <<
    round_to_int(players[index]->a_y) << "\r";
    //zabicie gracza
    ...
    synchro << "022 " << index << " " << players[index]->moving << " " <<
    players[index]->direction << "\r";
    synchro << "021 " << index << " " << players[index]->xmap << " " << players[index]->ymap <<
    "\r";
...
void GameEngine::update_map(int client_id) {
    ...
    for (unsigned int i = 0; i < items.size(); i++) {
        synchro2 << "040 " << items[i]->subclass << " " << items[i]->xmap << " " <<
        items[i]->ymap << "\r";
    }
    //dodanie graczy
    for (unsigned int i = 0; i < players.size(); i++) {
        synchro2 << "030 " << players[i]->subclass << " " << (int) players[i]->color.r << " " <<
        (int) players[i]->color.g << " " << (int) players[i]->color.b << " " << players[i]->name <<
        "\r";
        synchro2 << "021 " << i << " " << players[i]->xmap << " " << players[i]->ymap <<
        "\r";//bezwzględne umieszczenie na mapie
        synchro2 << "022 " << i << " " << players[i]->moving << " " << players[i]->direction <<
