...
void GameEngine::exec_cmd(string cmd) {
    ...
    if (cmd == "exit") {
        ...
        if (cmds.size() >= 2) {
            ...
            synchro2 << "026 " << index << " " << (int) players[index]->color.r << " " <<
            (int) players[index]->color.g << " " << (int) players[index]->color.b << "\r";
        ...
        for (unsigned int i = 0; i < players.size(); i++) {
            ...
            ss << ", kolor: (" << (int) players[i]->color.r << ", " << (int) players[i]->color.g <<
            ", " << (int) players[i]->color.b << ")";
            Log::info(ss.str());
        }
    } else if (cmds[0] == "mode") {
        if (cmds.size() == 2) {
        ...
        if (cmds.size() >= 3) {
            ...
            send_string += '\n';
            if (receiver_index == 0) {
                network->addtask_send_to_server(string_to_char(send_string),
                                                (int) send_string.length());
            } else {
                network->addtask_send_to_client(receiver_index, string_to_char(send_string),
                                                (int) send_string.length());
            }
        }
    } else if (cmds[0] ==
               "control") { //ustaw sterowanie graczem przez klienta: control [player_id] [client_id]
        if (cmds.size() >= 3) {
            ...
            stringstream ss2;
            ss2 << "Przypisano gracza " << player_id << " do klienta " << client_id;
            Log::info(ss2.str());
        }
        ...
        Log::info(
                "add [pacman,ghost] (ai [0-5]) - nowy gracz (wpisz \"add --help\" w celu uzyskania pomocy)");
        ...
        Log::info(
                "control [playerID] [clientID] - przydziel wybranemu klientowi sterowanie wybranym graczem");
        Log::info(
                "remote [0,clientID] [polecenie] - wykonaj zdalne polecenie na serwerze (ID=0) lub u klienta");
