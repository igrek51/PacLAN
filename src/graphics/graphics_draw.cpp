...
void Graphics::draw_menu() {
    ...
    (App::game_engine->menu_subclass == P_PACMAN ? App::lang->player_class_pacman
                                                 : App::lang->player_class_ghost);
    draw_text(ss.str(), font1, rgba(255, 255, 255), screen_w / 2, 170, TEXT_CENTER);
    ss_clear(ss);
    ss << App::lang->player_color << "(" << (int) App::game_engine->menu_color.r << ", " <<
    (int) App::game_engine->menu_color.g << ", " << (int) App::game_engine->menu_color.b << ")";
    ...
    App::game_engine->menu_pacman->draw_sprite_at(App::game_engine->menu_pacman->texture, 260,
                                                  148 + (App::game_engine->menu - 1) * 30);
    App::game_engine->menu_ghost->draw_sprite_at(App::game_engine->menu_ghost->texture, 540,
                                                 148 + (App::game_engine->menu - 1) * 30);
...
void Graphics::draw_round() {
    ...
    if (Config::show_grid) {
        ...
        for (int i = 0; i <= App::game_engine->map->grid_w; i++) {
            draw_line(App::game_engine->map->grid_space * i + App::game_engine->map->grid_x_pos, 0,
                      App::game_engine->map->grid_space * i + App::game_engine->map->grid_x_pos,
                      screen_h, ccolor);
        }
        for (int j = 0; j <= App::game_engine->map->grid_h; j++) {
            draw_line(0, App::game_engine->map->grid_space * j + App::game_engine->map->grid_y_pos,
                      screen_w,
                      App::game_engine->map->grid_space * j + App::game_engine->map->grid_y_pos,
                      ccolor);
        }
        //niedostępne pola
        for (int w = 0; w < App::game_engine->map->grid_h; w++) { //dla każdego wiersza
            for (int k = 0; k < App::game_engine->map->grid_w; k++) { //dla każdej kolumny
                if (App::game_engine->map->grid[w][k] == 'x') {
                    draw_fill_rect(App::game_engine->map->grid_space * k +
                                   App::game_engine->map->grid_x_pos,
                                   App::game_engine->map->grid_space * w +
                                   App::game_engine->map->grid_y_pos,
                                   App::game_engine->map->grid_space,
                                   App::game_engine->map->grid_space, ccolor);
                }
            }
        }
    }
            ...
            if (App::game_engine->players[i]->controlby == P_AI) { //dla AI
                if (App::game_engine->players[i]->sciezka != nullptr) {
                    for (unsigned int j = 0;
                         j < App::game_engine->players[i]->sciezka->points.size() - 1; j++) {
                        ...
                        if (abs(y1 - y2) == App::game_engine->map->grid_h - 1)
                            continue;
                        draw_line(App::game_engine->xmap_to_x(x1), App::game_engine->ymap_to_y(y1),
                                  App::game_engine->xmap_to_x(x2),
                                  App::game_engine->ymap_to_y(y2),
                                  App::game_engine->players[i]->color);
                    }
                }
            }
        }
    ...
    if (App::game_engine->round_next > 0) {
        ...
        if (App::game_engine->round_next > Config::next_round_time) {
            ...
            Timer::cycles_to_s(App::game_engine->round_next - Config::next_round_time);
        } else {
    ...
    if (42 + (App::game_engine->pacmans.size() - 1) * list_spacing >=
    ...
    for (unsigned int i = 0; i < App::game_engine->pacmans.size(); i++) {
        ...
        draw_text(ss.str(), font1, App::game_engine->pacmans[i]->color, 30, 30 + i * list_spacing,
                  TEXT_LEFT);
        //obrazek gracza
        App::game_engine->pacmans[i]->draw_sprite_at(App::game_engine->pacmans[i]->texture, 14,
                                                     38 + i * list_spacing);
    }
    ss_clear(ss);
    ...
    for (unsigned int i = 0; i < App::game_engine->ghosts.size(); i++) {
        ...
        draw_text(ss.str(), font1, App::game_engine->ghosts[i]->color, screen_w - 30,
