...
void Ghost::ai_control() {
        ...
        if (sciezka == nullptr) {
            ...
            if (ai_level == 3) {
                ...
                for (unsigned int i = 0; i < game_engine->players.size(); i++) {
                    if (game_engine->players[i]->subclass == P_PACMAN &&
    ...
    if (ai_level == 2 || ai_level == 4 || ai_level == 5) {
        ...
        if (ai_level == 2 && (min_d > 10 || min_i ==
                                            -1)) { //gdy pacmany są w odległości większej niż LIMIT_D lub nie ma pacmanów
            ...
            for (unsigned int i = 0; i < game_engine->items.size(); i++) {
                if (game_engine->items[i]->subclass == I_SMALLDOT ||
                    ...
                    //odległość w metryce miejskiej
                    if (min_d == -1 ||
                        game_engine->distance_m(this, game_engine->items[i]) < min_d) {
