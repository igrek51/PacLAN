...
void Pacman::ai_control() {
    ...
    if (min_i != -1 && game_engine->eating >= Config::eating_time_critical) {
    ...
    } else if (min_i != -1 && (min_d <= Config::ai_pacman_start_escape ||
                               esc_histereza == 1)) { //gdy duszki są w odległości <= LIMIT_D
        //uciekaj od duszka
        escape = true;
        esc_histereza = 1;
        target_x = game_engine->players[min_i]->xmap;
        ...
        for (unsigned int i = 0; i < game_engine->items.size(); i++) {
            if (game_engine->items[i]->subclass == I_SMALLDOT ||
                game_engine->items[i]->subclass == I_LARGEDOT) {
                //odległość w metryce miejskiej
        ...
        if (sciezka->points.size() > 1) {
            ...
            if (escape) { //jeśli ucieka od celu
                ...
                for (int d = 0; d < 3; d++) {
                    ...
                    suma = 0;
                    for (unsigned int i = 0; i < game_engine->players.size(); i++) {
                        if (game_engine->players[i]->subclass == P_GHOST &&
                            game_engine->players[i]->respawn == 0) {
                            ...
                            if (game_engine->map->grid_w - dist <
                                dist) //jeśli odległość jest większa od połowy mapki
