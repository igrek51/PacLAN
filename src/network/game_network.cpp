...
void GameEngine::network_packet_process(int from, string packet) {
    ...
    if (p[0] == "001") { // polecenie zdalne: 001 [polecenie zdalne]
        ...
        synchro2 << "100 " << menu_subclass << " " << (int) menu_color.r << " " <<
        (int) menu_color.g << " " << (int) menu_color.b << " " << menu_name << "\r";
        ...
        if (p.size() >= 3) {
            ...
            players[player_i]->name = new_name.str();
               ...
               "028") { // zmień następny kierunek i ruch: 028 [index_playera] [next_moving] [next_direction]
        if (p.size() >= 4) {
            int player_i = atoi(p[1].c_str());
            if (player_i < 0 || player_i >= (int) players.size()) return;
            ...
            players[player_i]->next_direction = next_direction;
               ...
               "030") { //   LISTA GRACZY: dodaj nowego gracza: 030 [subclass] [color.r] [color.g] [color.b] [nazwa gracza]
        if (p.size() >= 6) {
        ...
        if (p.size() >= 4) {
            ...
            for (unsigned int item_index = 0; item_index < items.size(); item_index++) {
                if (items[item_index]->subclass == item_type && items[item_index]->xmap == xmap &&
                    items[item_index]->ymap == ymap) {
        ...
        items.clear();
               ...
               "050") { //   ANIMACJE: utworzenie animacji śmierci: 050 [player_index] [x] [y]
        if (p.size() >= 4) {
            ...
            graphics->animations.push_back(
                    new DeathAnimation(x, y, to_copy, players[player_index]->color, clip,
                                       graphics));
        }
    } else if (p[0] ==
               "100") { // KOMUNIKATY OD KLIENTA: prośba dodania nowego gracza od klienta i przyporządkowania do klienta: 100 [subclass] [color.r] [color.g] [color.b] [nazwa gracza]
        if (p.size() >= 6) {
            if (network->server) { //odpowiedź serwera
                ...
                Player* nowy = add_player(subclass, name.str(),
                                          Graphics::rgba(color_r, color_g, color_b), P_LAN);
                nowy->lan_id = from;
            ...
            if (network->server) { //odpowiedź serwera
                ...
                for (unsigned int i = 0; i < players.size(); i++) {
                    if (players[i]->lan_id == from) {
                        ...
                        synchro2 << "028 " << i << " " << players[i]->next_moving << " " <<
                        players[i]->next_direction << "\r";
                    }
                }
                ...
                for (unsigned int i = 1; i < network->connections.size(); i++) {
                    ...
                    network->addtask_send_to_client(i, string_to_char(synchro2.str()),
                                                    (int) synchro2.str().length());
                }
            }
        }
    } else if (p[0] ==
               "102") { // sygnał zmiany kierunku od klienta (moving=1): 102 [next_direction]
        if (p.size() >= 2) {
            if (network->server) { //odpowiedź serwera
                if (from <= 0 || from >= (int) network->connections.size()) return;
